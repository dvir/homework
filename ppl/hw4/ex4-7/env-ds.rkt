...
;; Closure:
; Type: [LIST(Symbol)*LIST*Env -> LIST]
(define make-procedure-variadic
  (lambda (parameters body env)
    (attach-tag (list (list parameters) body env) 'procedure-variadic)))

; Type: [T -> Boolean]
(define compound-procedure-variadic?
  (lambda (p)
    (tagged-by? p 'procedure-variadic)))

; Type: [LIST -> LIST(Symbol)]
(define procedure-variadic-parameters
  (lambda (p)
    (car (get-content p))))

; Type: [LIST -> LIST]
(define procedure-variadic-body
  (lambda (p)
    (cadr (get-content p))))

; Type: [LIST -> Env]
(define procedure-variadic-environment
  (lambda (p)
    (caddr (get-content p))))
...
(define procedure?
  (lambda (p)
    (or (primitive-procedure? p) (compound-procedure? p) (compound-procedure-variadic? p))))
