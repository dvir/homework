...
(define cond->if$ (lambda (exp cont)
    (let* (
           ...
           (first (if (cond-empty-clauses? clauses) null (cond-first-clause clauses)))
           (rest (if (cond-empty-clauses? clauses) null (cond-rest-clauses clauses)))
         )
      (cond
        ((cond-empty-clauses? clauses) (cont null))
        ((and (cond-empty-clauses? rest) (cond-else-clause? first)) (cont (sequence->exp (cond-actions first))))
        ((cond-else-clause? first) (error 'cond->if$ "ELSE clause isn't last: ~s" clauses)) ; fail
        (else 
           (cond->if$ 
              (make-cond rest)
              (lambda(res-rest)
