...
public class IRCProtocol<T> implements AsyncServerProtocol<T> {
    /**
    ...
    public T processMessage(T msgRaw) {
        ...
        if (msg.length() == 0) {
            ...
            ((StringMessage) msgRaw).setMessage("");
            return msgRaw;
        }
        ...
        if (command.equals("NICK") || command.equals("USER")) {
            ...
            if (!_user.isRegistered()) {
                response = errorReply(IRCProtocol.<T>ERR.NOTREGISTERED, _user);
            }
        ...
        ((StringMessage) msgRaw).setMessage(response);
        return msgRaw;
    }
